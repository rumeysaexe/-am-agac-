...
WORKDIR /src
COPY ["çam agacı.csproj", "."]
RUN dotnet restore "./çam agacı.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./çam agacı.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./çam agacı.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "çam agacı.dll"]